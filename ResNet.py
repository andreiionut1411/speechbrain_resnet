...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        pool_out = math.ceil((input_size + 1) / 2)
        input_out = math.ceil(
            pool_out / (strides[0] * strides[1] * strides[2] * strides[3])
        )

        self.conv1 = nn.Conv2d(1, channels[0], 7, 1, 1, bias=False)
        ...
        self.layer3 = self._make_layer_se(
            channels[1], channels[2], block_sizes[2], stride=strides[2]
        )
        self.layer4 = self._make_layer_se(
